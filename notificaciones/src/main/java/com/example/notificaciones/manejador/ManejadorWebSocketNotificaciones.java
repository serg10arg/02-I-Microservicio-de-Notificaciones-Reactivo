...
import java.util.concurrent.ConcurrentHashMap; // Para gestionar las sesiones concurrentemente
...
public class ManejadorWebSocketNotificaciones implements WebSocketHandler {
    ...
    // ConcurrentHashMap es thread-safe y adecuada para entornos concurrentes.
    private final Map<String, WebSocketSession> sesionesActivas = new ConcurrentHashMap<>();

    private final ObjectMapper objectMapper; // Para convertir objetos Notificacion a JSON

    public ManejadorWebSocketNotificaciones(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper; // Inicializa ObjectMapper
    }
        ...
        return ReactiveSecurityContextHolder.getContext() // Obtiene el contexto de seguridad reactivo
                ...
                .flatMap(auth -> {
                    ...
                    if (esAutenticadoYCoincide || (esAdministrador && identificadorUsuario.equals("admin_monitor"))) { // Ejemplo de un canal para admin
                        ...
                        sesionesActivas.put(identificadorUsuario, sesion); // Registra la sesión activa
                        ...
                        Mono<Void> salida = sesion.closeStatus()
                                .doOnNext(estado -> {
                                    ...
                                    sesionesActivas.remove(identificadorUsuario); // Elimina la sesión al desconectarse
                                })
                                .then();
    ...
    public Mono<Void> enviarNotificacionAUsuario(String identificadorUsuario, Notificacion notificacion) {
        ...
        if (sesion != null && sesion.isOpen()) {
            try {
                String notificacionJson = objectMapper.writeValueAsString(notificacion); // Convierte la notificación a JSON
